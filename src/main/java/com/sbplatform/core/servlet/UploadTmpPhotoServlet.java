package com.sbplatform.core.servlet;
		...
		try {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (!item.isFormField() && item.getName().length() > 0) {
					...
					// com.sbplatform.core.util.LogUtil.info(item.getName()+"=="+UploadTool.createPhotoID()+suffixName+"=="+UploadTool.FormetFileSize(item.getSize())+"savePath"+savePath);
