...
import com.sbplatform.core.jackson.annotation.AllowProperty;
...
import com.sbplatform.core.util.StringHelper;
...
public class JavassistFilterPropertyHandler implements FilterPropertyHandler {
	 ...
	 */
	private void processIgnorePropertiesAnnotation(FilterProperties properties, Map<Class<?>, Collection<String>> pojoAndNamesMap) {
		IgnoreProperty[] values = properties.ignore();
	...
	private void processIgnorePropertyAnnotation(IgnoreProperty property, Map<Class<?>, Collection<String>> pojoAndNamesMap) {
		String[] names = property.property();
		Class<?> pojoClass = property.clazz();
	...
	private void processAllowPropertyAnnotation(AllowProperty property, Map<Class<?>, Collection<String>> pojoAndNamesMap) {
		String[] allowNames = property.property();
		Class<?> pojoClass = property.clazz();

		Collection<String> ignoreProperties = EntityHelper.getUnstaticClassFieldNameCollection(pojoClass,true);
	...
	public Map<Class<?>, Class<?>> getProxyMixInAnnotation(Method method) {
		...
		FilterProperties classIgnoreProperties = clazzOfMethodIn.getAnnotation(FilterProperties.class);
		...
		FilterProperties methodIgnoreProperties = method.getAnnotation(FilterProperties.class);
		IgnoreProperty methodIgnoreProperty = method.getAnnotation(IgnoreProperty.class);
		AllowProperty methodAllowProperty = method.getAnnotation(AllowProperty.class);

		if (methodAllowProperty != null) {// 方法上的AllowProperty注解
			processAllowPropertyAnnotation(methodAllowProperty, pojoAndNamesMap);
		}
		if (classAllowProperty != null) {// 类上的AllowProperty注解
		...
		if (methodIgnoreProperties != null) {// 方法上的IgnoreProperties注解
			processIgnorePropertiesAnnotation(methodIgnoreProperties, pojoAndNamesMap);
		}
		if (methodIgnoreProperty != null) {// 方法上的IgnoreProperties注解
			processIgnorePropertyAnnotation(methodIgnoreProperty, pojoAndNamesMap);
		}
