...
public class UploadUtils {
	...
	private String validateFields(HttpServletRequest request) {
		...
		if (contentType == null || !contentType.startsWith("multipart")) {
			// TODO
			com.sbplatform.core.util.LogUtil.info("请求不包含multipart/form-data流");
			errorInfo = "请求不包含multipart/form-data流";
		} else if (maxSize < contentLength) {
			// TODO
			com.sbplatform.core.util.LogUtil.info("上传文件大小超出文件最大大小");
	...
	private String saveFile(FileItem item) {
		...
		if (item.getSize() > maxSize) { // 检查文件大小
			...
			try {
				...
				e.printStackTrace();
				com.sbplatform.core.util.LogUtil.info("上传失败了！！！");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
