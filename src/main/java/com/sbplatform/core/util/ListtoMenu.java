...
public class ListtoMenu {
	 ...
	 * @return
	 */
	public static String getMenu(List<MenuFunction> set, List<MenuFunction> set1) {
		StringBuffer buffer = new StringBuffer();
		buffer.append("{\"menus\":[");
		for (MenuFunction node : set) {
			String iconClass = "default";// 权限图标样式
			if (node.getIcon() != null) {
				iconClass = node.getIcon().getIconClass();
			}
	...
	static void iterGet(List<MenuFunction> set1, String pid, StringBuffer buffer) {

		for (MenuFunction node : set1) {

			// 查找所有父节点为pid的所有对象，然后拼接为json格式的数据
			count++;
			if (node.getParent().getId().equals(pid))

			{
				buffer.append("{\"menuid\":\"" + node.getId()
						+ " \",\"icon\":\"" + node.getIcon().getIconClass()
	 ...
	 * @return
	 */
	public static String getBootMenu(List<MenuFunction> pFunctions,
			List<MenuFunction> functions) {
		StringBuffer menuString = new StringBuffer();
		menuString.append("<ul>");
		for (MenuFunction pFunction : pFunctions) {
			menuString
					.append("<li><a href=\"#\"><span class=\"icon16 icomoon-icon-stats-up\"></span><b>"
							+ pFunction.getName() + "</b></a>");
			int submenusize = pFunction.getSubs().size();
			if (submenusize == 0) {
			...
			for (MenuFunction function : functions) {
				...
				if (function.getParent().getId().equals(pFunction.getId())) {
					menuString
							.append("<li><a href=\""
									+ function.getUrl()
									+ "\" target=\"contentiframe\"><span class=\"icon16 icomoon-icon-file\"></span>"
									+ function.getName() + "</a></li>");
				}
			}
			if (submenusize > 0) {
	 ...
	 * @return
	 */
	public static String getEasyuiMenu(List<MenuFunction> pFunctions,
			List<MenuFunction> functions) {
		StringBuffer menuString = new StringBuffer();
		for (MenuFunction pFunction : pFunctions) {
			menuString.append("<div  title=\"" + pFunction.getName()
					+ "\" iconCls=\"" + pFunction.getIcon().getIconClass()
					+ "\">");
			int submenusize = pFunction.getSubs().size();
			if (submenusize == 0) {
			...
			for (MenuFunction function : functions) {
				...
				if (function.getParent().getId().equals(pFunction.getId())) {
					...
					if (function.getIcon() != null) {
						icon = function.getIcon().getIconClass();
					}
					...
					menuString.append("<li><div onclick=\"addTab(\'"
							+ function.getName() + "\',\'"
							+ function.getUrl() + "&clickFunctionId="
							+ function.getId() + "\',\'" + icon
	...
	public static String getEasyuiMultistageMenu(
			Map<Integer, List<MenuFunction>> map) {
		StringBuffer menuString = new StringBuffer();
		List<MenuFunction> list = map.get(0);
		for (MenuFunction function : list) {
			menuString.append("<div   title=\"" + function.getName()
					+ "\" iconCls=\"" + function.getIcon().getIconClass()
					+ "\">");
			int submenusize = function.getSubs().size();
			if (submenusize == 0) {
				menuString.append("</div>");
			}
			if (submenusize > 0) {
	 ...
	 * @return
	 */
	private static String getChild(MenuFunction parent,int level,Map<Integer, List<MenuFunction>> map){
		StringBuffer menuString = new StringBuffer();
		List<MenuFunction> list = map.get(level);
		for (MenuFunction function : list) {
			if (function.getParent().getId().equals(parent.getId())){
				if(function.getSubs().size()==0||!map.containsKey(level+1)){
	 ...
	 * @return
	 */
	private static String getChildOfTree(MenuFunction parent,int level,Map<Integer, List<MenuFunction>> map){
		StringBuffer menuString = new StringBuffer();
		List<MenuFunction> list = map.get(level);
		for (MenuFunction function : list) {
			if (function.getParent().getId().equals(parent.getId())){
				if(function.getSubs().size()==0||!map.containsKey(level+1)){
					...
					menuString.append("<li state=\"closed\" iconCls=\"" + function.getIcon().getIconClass()+"\" ><span>"+function.getName()+"</span>");
					menuString.append("<ul >");
	 ...
	 * @return
	 */
	private static String getLeaf(MenuFunction function){
		StringBuffer menuString = new StringBuffer();
		String icon = "folder";
		if (function.getIcon() != null) {
			icon = function.getIcon().getIconClass();
		...
		menuString.append("\',\'");
		menuString.append(function.getUrl());
	 ...
	 * @return
	 */
	private static String getLeafOfTree(MenuFunction function){
		StringBuffer menuString = new StringBuffer();
		String icon = "folder";
		if (function.getIcon() != null) {
			icon = function.getIcon().getIconClass();
	 ...
	 * @return
	 */
	public static String getBootstrapMenu(Map<Integer, List<MenuFunction>> map) {
		StringBuffer menuString = new StringBuffer();
		menuString.append("<ul class=\"nav\">");
		List<MenuFunction> pFunctions = (List<MenuFunction>) map.get(0);
		if(pFunctions==null || pFunctions.size()==0){
			return "";
		}
		for (MenuFunction pFunction : pFunctions) {
			//是否有子菜单
			boolean hasSub = pFunction.getSubs().size()==0?false:true;
			
			//绘制一级菜单
			menuString.append("	<li class=\"dropdown\"> ");
			menuString.append("		<a href=\"javascript:;\" class=\"dropdown-toggle\" data-toggle=\"dropdown\"> ");
			menuString.append("			<span class=\"bootstrap-icon\" style=\"background-image: url('"+pFunction.getIcon().getIconPath()+"')\"></span> "+pFunction.getName()+" ");
			if(hasSub){
	...
	* @throws
	 */
	private static String getBootStrapChild(MenuFunction parent,int level,Map<Integer, List<MenuFunction>> map){
		StringBuffer menuString = new StringBuffer();
		List<MenuFunction> list = map.get(level);
		...
		for (MenuFunction function : list) {
			if (function.getParent().getId().equals(parent.getId())){
				boolean hasSub = function.getSubs().size()!=0 && map.containsKey(level+1);
				String menu_url = function.getUrl();
				if(StringUtils.isNotEmpty(menu_url)){
					menu_url += "&clickFunctionId="+function.getId();
				}
				menuString.append("		<li onclick=\"showContent(\'"+function.getName()+"\',\'"+menu_url+"\')\"  title=\""+function.getName()+"\" url=\""+function.getUrl()+"\" ");
				if(hasSub){
