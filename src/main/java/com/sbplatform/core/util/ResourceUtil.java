...
public class ResourceUtil {
	...
	@Deprecated
	public static final List<RoleMenuFunction> getSessionRoleFunction(String roleId) {
		HttpSession session = ContextHolderUtils.getSession();
		if (session.getAttributeNames().hasMoreElements()) {
			List<RoleMenuFunction> roleMenuFunctions = (List<RoleMenuFunction>)session.getAttribute(roleId);
			if (roleMenuFunctions != null) {
				return roleMenuFunctions;
			} else {
				return null;
			}
		} else {
...
//    update-end--Author:zhangguoming  Date:20140226 for：添加验证码
	...
	public static void main(String[] args) {
		com.sbplatform.core.util.LogUtil.info(getPorjectPath());
		com.sbplatform.core.util.LogUtil.info(getSysPath());
