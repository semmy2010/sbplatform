...
public class UploadTag extends TagSupport {
	...
	public StringBuffer end() {
		...
		sb.append("<script type=\"text/javascript\">" + "var flag = false;" + "var fileitem=\"\";" + "var fileKey=\"\";" + "var serverMsg=\"\";" + "var m = new Map();" + "$(function(){"
				+ "$(\'#"
				+ id
				+ "\').uploadify({"
				+ "buttonText:\'"
				...
				+ "\',"
				+ "fileSizeLimit:\'15MB\',"
				+ "swf:\'plug-in/uploadify/uploadify.swf\',	" + "uploader:\'" + getUploader() + "onUploadStart : function(file) { ");
		if (formData != null) {
		...
		if (dialog) {
			sb.append("var win = frameElement.api.opener;" + "win.reloadTable();" + "win.tip(serverMsg);" + "frameElement.api.close();");
		} else {
			if (callback != null)
				sb.append("" + callback + "();");
		}
		...
		if (view) {
			sb.append("var fileitem=\"<span id=\'\"+d.expandData.id+\"\'><a href=\'#\' onclick=openwindow(\'文件查看\',\'\"+d.expandData.href+\"\',\'70%\',\'80%\') title=\'查看\'>\"+d.expandData.name+\"</a><img border=\'0\' onclick=confuploadify(\'\"+d.expandData.delurl+\"\',\'\"+d.expandData.id+\"\') title=\'删除\' src=\'plug-in/uploadify/img/uploadify-cancel.png\' widht=\'15\' height=\'15\'>&nbsp;&nbsp;</span>\";");
			sb.append("m.put(d.expandData.id,fileitem);");
			sb.append("fileKey=d.expandData.fileKey;");
		}
		if (onUploadSuccess != null) {
			sb.append(onUploadSuccess + "(file,d,response);");
		}
		sb.append("if(d.status==0){");
		sb.append("var win = frameElement.api.opener;");
		//				sb.append("win.tip(d.msg);");
		sb.append("serverMsg = d.message;");
		sb.append("}");
		sb.append("},");
		...
		sb.append("break;");
		sb.append("case -110:" + "tip(\"文件 [\"+file.name+\"] 大小超出系统限制的\"+$(\'#" + id + "\').uploadify(\'settings\',\'fileSizeLimit\')+\"大小！\");" + "break;" + "case -120:"
		...
		sb.append("}," + "onUploadProgress : function(file, bytesUploaded, bytesTotal, totalBytesUploaded, totalBytesTotal) { "
				...
				+ "}" + "});" + "});" + "function upload() {" + "	$(\'#" + id + "\').uploadify('upload', '*');" + "		return flag;" + "}" + "function cancel() {" + "$(\'#" + id
