...
import com.sbplatform.core.util.ApplicationContextUtil;
...
import com.sbplatform.tag.vo.easyui.ColumnValue;
import com.sbplatform.tag.vo.easyui.DataGridColumn;
import com.sbplatform.tag.vo.easyui.DataGridUrl;
import com.sbplatform.tag.vo.easyui.OptTypeDirection;
...
public class DataGridTag extends TagSupport {
	 ...
	 * 设置编辑操作URL
	 */
	public void setEditUrl(String url, String title, String message, String exp, String funname, String operationCode) {
		DataGridUrl dataGridUrl = new DataGridUrl();
		dataGridUrl.setTitle(title);
		dataGridUrl.setUrl(url);
		dataGridUrl.setType(OptTypeDirection.Edit);
		dataGridUrl.setMessage(message);
		dataGridUrl.setExp(exp);
	...
	public void setColumn(String title, String field, Integer width, String rowspan, String colspan, String align, boolean sortable, boolean checkbox, String formatter, boolean hidden,
			String replace, String treefield, boolean image, String imageSize, boolean query, String url, String funname, String arg, String queryMode, String dictionary,
			boolean frozenColumn, String extend, String style, String downloadName, boolean isAuto, String extendParams) {
		...
		if (!StringUtils.isBlank(dictionary)) {
			if (dictionary.contains(",")) {
				...
				String text = "";
				String value = "";
				List<Type> typeList = TypeGroup.allTypes.get(dictionary.toLowerCase());
	...
	public StringBuffer datatables() {
		...
		sb.append("\"oLanguage\" : {" + "\"sLengthMenu\" : \" _MENU_ 条记录\"," + "\"sZeroRecords\" : \"没有检索到数据\"," + "\"sInfo\" : \"第 _START_ 至 _END_ 条数据 共 _TOTAL_ 条\","
				+ "\"sInfoEmtpy\" : \"没有数据\"," + "\"sProcessing\" : \"正在加载数据...\"," + "\"sSearch\" : \"搜索\"," + "\"oPaginate\" : {" + "\"sFirst\" : \"首页\"," + "\"sPrevious\" : \"前页\", "
		...
		sb.append("]" + "});" + "});" + "</script>");
		sb.append("<table id=\"" + name + "\" name=\"" + name + "\" width=\"100%\"  class=\"" + this.style + "\" id=\"" + this.name + "\" toolbar=\"#" + this.name + "tb\"></table>");
		return sb;
	...
	public StringBuffer end() {
		...
		sb.append("try{");
		sb.append("	$(\'#" + this.name + "\').datagrid(\'reload\');");
		sb.append("	$(\'#" + this.name + "\').treegrid(\'reload\');");
		sb.append("}catch(ex){}");
		sb.append("}");
		...
		sb.append("function getSelected(field){" + "var row = $(\'#\'+gridname)." + grid + "(\'getSelected\');" + "if(row!=null)" + "{" + "value= row[field];" + "}" + "else" + "{"
				+ "value=\'\';" + "}" + "return value;" + "}");
		sb.append("function get" + this.name + "Selections(field){" + "var ids = [];" + "var rows = $(\'#" + this.name + "\')." + grid + "(\'getSelections\');"
		...
		if (this.columnList.size() > 0) {
			...
			sb.append("$(\'#" + this.name + "SearchBar\').find('*').each(function(){queryParams[$(this).attr('name')]=$(this).val();});");
			...
			sb.append(" $(\"#\"+name+\"SearchBar\").find(\":input\").val(\"\");");
		...
		if (hasQueryColum(this.columnList)) {
			sbQuery.append("<div name=\"searchColums\" style=\"margin:5px 5px;\">");
			// 如果表单是组合查询
			if ("group".equals(getQueryMode())) {
				for (DataGridColumn col : this.columnList) {
					if (col.isQuery()) {
						sbQuery.append("<span style=\"display:-moz-inline-box;display:inline-block;\">");
						sbQuery.append("<span style=\"display:-moz-inline-box;display:inline-block;width: 80px;text-align:right;text-overflow:ellipsis;-o-text-overflow:ellipsis; overflow: hidden;white-space:nowrap; \" title=\""
								+ col.getTitle() + "\">" + col.getTitle() + "：</span>");
						if ("single".equals(col.getQueryMode())) {
							if (!StringUtil.isEmpty(col.getReplace())) {
								sbQuery.append("<select name=\"" + col.getField().replaceAll("_", "\\.") + "\" width=\"100\" style=\"width: 104px\"> ");
								sbQuery.append("<option value =\"\" >---请选择---</option>");
								...
								for (String string : test) {
									...
									sbQuery.append("<option value =\"" + value + "\">" + text + "</option>");
								}
								...
								if (col.getDictionary().contains(",")) {
									...
									sbQuery.append("<option value =\"\" >---请选择---</option>");
									for (Map<String, Object> map : list) {
										sbQuery.append(" <option value=\"" + map.get("field") + "\">");
										sbQuery.append(map.get("text"));
										sbQuery.append(" </option>");
									}
									sbQuery.append("</select>");
								} else {
									Map<String, List<Type>> typedatas = TypeGroup.allTypes;
									List<Type> types = typedatas.get(col.getDictionary().toLowerCase());
									sbQuery.append("<select name=\"" + col.getField().replaceAll("_", "\\.") + "\" WIDTH=\"100\" style=\"width: 104px\"> ");
									sbQuery.append("<option value =\"\" >---请选择---</option>");
									for (Type type : types) {
										sbQuery.append(" <option value=\"" + type.getCode() + "\">");
										sbQuery.append(type.getName());
										sbQuery.append(" </option>");
									}
									sbQuery.append("</select>");
								}
							} else if (col.isAutocomplete()) {
								sbQuery.append(getAutoSpan(col.getField().replaceAll("_", "\\."), extendAttribute(col.getExtend())));
							} else {
								sbQuery.append("<input type=\"text\" name=\"" + col.getField().replaceAll("_", "\\.") + "\"  " + extendAttribute(col.getExtend())
										+ " style=\"width: 100px\" />");
							}
						} else if ("group".equals(col.getQueryMode())) {
							sbQuery.append("<input type=\"text\" name=\"" + col.getField() + "_begin\"  style=\"width: 94px\" " + extendAttribute(col.getExtend()) + "/>");
							sbQuery.append("<span style=\"display:-moz-inline-box;display:inline-block;width: 8px;text-align:right;\">~</span>");
							sbQuery.append("<input type=\"text\" name=\"" + col.getField() + "_end\"  style=\"width: 94px\" " + extendAttribute(col.getExtend()) + "/>");
						}
						sbQuery.append("</span>");
					}
				}
			...
			sb.append("<script type=\"text/javascript\">");
			sb.append("$(function(){");
			sb.append("if($('#" + this.name + "SearchBar')){");
			sb.append("$('#" + this.name + "SearchBar').remove();");
			sb.append("}");
			sb.append("});");
			sb.append("</script>");
		}
		...
		if ("group".equals(getQueryMode()) && hasQueryColum(this.columnList)) {// 如果表单是组合查询
			sbQuery.append("<div style=\"text-align:center;margin:5px 5px;\">");
			sbQuery.append("<a href=\"#\" class=\"easyui-linkbutton\" iconCls=\"icon-search\" onclick=\"" + this.name + "search()\">查询</a>");
			sbQuery.append("&nbsp;&nbsp;");
			sbQuery.append("<a href=\"#\" class=\"easyui-linkbutton\" iconCls=\"icon-reload\" onclick=\"searchReset(\\'" + this.name + "\\')\">重置</a>");
			sbQuery.append("</div>");
		} else if ("single".equals(getQueryMode()) && hasQueryColum(this.columnList)) {// 如果表单是单查询
			sb.append("<div style=\"float:right;\">");
			sb.append("<input id=\"" + this.name + "searchbox\" class=\"easyui-searchbox\"  data-options=\"searcher:" + this.name + "searchbox,prompt:\'请输入关键字\',menu:\'#" + this.name
					+ "mm\'\"></input>");
			sb.append("<div id=\"" + this.name + "mm\" style=\"width:120px\">");
			for (DataGridColumn col : this.columnList) {
				if (col.isQuery()) {
					sb.append("<div data-options=\"name:\'" + col.getField().replaceAll("_", "\\.") + "\',iconCls:\'icon-ok\' " + extendAttribute(col.getExtend()) + " \">" + col.getTitle()
							+ "</div>  ");
				}
			}
			sb.append("</div>");
		...
		sb.append("$(function(){");
		sb.append("if($('#" + this.name + "SearchBar')){");
		sb.append("$('#" + this.name + "SearchBar').html('" + sbQuery + "');");
		sb.append("}");
		sb.append("});");
		sb.append("</script>");
		//start  表格外围
		//sb.insert(0,"<div class=\"easyui-layout\" data-options=\"fit:true\"><div data-options=\"region:'north',title:'查找',split:false\" style=\"height: 110px;\" id=\""+this.name+"SearchBar\"></div><div data-options=\"region:'center',border:false\">");
		//sb.append("</div></div>");
		//end  表格外围
		return sb;
	}
	...
	protected void getOptUrl(StringBuffer sb) {
		...
		for (DataGridUrl dataGridUrl : list) {
			...
			if (OptTypeDirection.Edit.equals(dataGridUrl.getType())) {
				sb.append("href+=\"[<a href=\'#\' onclick=editRow('\"+rec.id+\"','" + this.title + "','" + url + "',null,null)>\";");
			}
			...
			if (OptTypeDirection.OpenWin.equals(dataGridUrl.getType())) {
				sb.append("href+=\"[<a href=\'#\' onclick=openwindow('" + dataGridUrl.getTitle() + "','" + url + "','" + this.name + "'," + dataGridUrl.getWidth() + ","
	...
	protected void getFun(StringBuffer sb, DataGridColumn column) {
		...
		if (this.columnValueList.size() > 0 && !column.getField().equals("opt")) {
			String value = getReplaceValue(column);
			if (!value.equals("")) {
				sb.append(value);
			}
		}
		sb.append("var href=\"<a style=\'color:blue\' href=\'#\' onclick=" + column.getFunname() + "('" + column.getTitle() + "','" + url + "')>\";");
		sb.append("return href+value+\'</a>\';");

	}

	private String getReplaceValue(DataGridColumn column) {
		String value = "";
		for (ColumnValue columnValue : this.columnValueList) {
			if (columnValue.getName().equals(column.getField())) {
				String[] values = columnValue.getValue().split(",");
				String[] text = columnValue.getText().split(",");
				for (int j = 0; j < values.length; j++) {
					value += "if(value=='" + values[j] + "'){value='" + text[j] + "'}";
				}
			}
		}
		return value;
		...
		for (DataGridColumn column : this.columnList) {
			...
			if ((column.isFrozenColumn() && frozen == 0) || (!column.isFrozenColumn() && frozen == 1)) {
				...
				if (column.getFormatter() != null) {
					sb.append(",formatter:function(value,rec,index){");
					sb.append("console.info(value);");
					// sb.append(" return new Date().format('" +
					// column.getFormatter() + "',value);}");
					sb.append(" return formatCSTDate(value,'" + column.getFormatter() + "');}");
				}
				// 加入操作
				if (column.getField().equals("opt")) {
					sb.append(",formatter:function(value,rec,index){");
				...
				// 值替換
				if (this.columnValueList.size() > 0 && !column.getField().equals("opt") && column.getUrl() == null) {
					String testString = "";
					for (ColumnValue columnValue : this.columnValueList) {
						if (columnValue.getName().equals(column.getField())) {
	...
	public String getNoAuthOperButton() {
		...
		if (ResourceUtil.getSessionUserName().getUserName().equals("admin") || !Globals.BUTTON_AUTHORITY_CHECK) {
			...
			if (nolist != null && nolist.size() > 0) {
				for (String s : nolist) {
					sb.append("$('#" + this.name + "tb\').find(\"" + s.replaceAll(" ", "") + "\").hide();");
				}
			}
		}
		com.sbplatform.core.util.LogUtil.info("----getNoAuthOperButton-------" + sb.toString());
		return sb.toString();
	}
