package com.sbplatform.web.cgform.service.impl.config.util;
...
import com.sbplatform.web.cgform.exception.DBException;
...
public class DbTableProcess {
	...
	public static void createOrDropTable(CgFormHeadEntity table, Session session) throws Exception {
		Template t;
		t = getConfig("/com/sbplatform/web/cgform/engine/hibernate").getTemplate("tableTemplate.ftl");
...
//       .setProperty("hibernate.connection.username",propertiesUtil.readProperty("jdbc.username.sb"))
//       .setProperty("hibernate.connection.password",propertiesUtil.readProperty("jdbc.password.sb"))  
//       .setProperty("hibernate.dialect",propertiesUtil.readProperty("hibernate.dialect"))
//       .setProperty("hibernate.connection.url",propertiesUtil.readProperty("jdbc.url.sb"))
...
//       
	...
	private int updateFieldName(String columnName,String id,Session session){
		return   session.createSQLQuery("update cgform_t_field set old_field_name= '"+columnName+"' where id='"+id+"'").executeUpdate();
	}
	/**
	 * 判断是不数字,不是数字的话加上''
	 *@Author JueYue
