package com.sbplatform.web.cgform.controller.generate;
...
import com.sbplatform.codegenerate.generate.onetomany.CgformCodeGenerateOneToMany;
...
import com.sbplatform.web.cgform.entity.generate.GenerateSubListEntity;
...
public class GenerateController extends BaseController {
	...
	public ModelAndView gogenerate(CgFormHeadEntity cgFormHead, HttpServletRequest request) {
		...
		if (cgFormHead.getJformType() == 1 || cgFormHead.getJformType() == 3) {
			//如果是单表或者附表，则进入单表模型
			returnModelAndView = "sb/cgform/generate/single";
		} else {
			//如果是主表，则进入一对多模型
			...
			if (StringUtil.isNotEmpty(cgFormHead.getSubTableStr())) {
				...
				for (String subTable : subTables) {
					...
					entityNameMap.put(subHead.getTableName(), SbReadTable.formatFieldCapital(subHead.getTableName()));
				}
			}
			request.setAttribute("subTableList", subTableList);
			returnModelAndView = "sb/cgform/generate/one2many";
		}
		...
		String entityName = SbReadTable.formatFieldCapital(cgFormHead.getTableName());
	...
	public void dogenerate(CgFormHeadEntity cgFormHead, GenerateEntity generateEntity, CreateFileProperty createFileProperty, HttpServletRequest request, HttpServletResponse response)
		...
		try {
			//step.2 判断表是否存在
			boolean tableexist = new SbReadTable().checkTableExist(tableName);
			if (tableexist) {
				//step.3 调用代码生成器
				new CgformCodeGenerate(createFileProperty, generateEntity).generateToFile();
				j.setMessage("成功生成增删改查->功能：" + ftlDescription);
			} else {
				j.setMessage("表[" + tableName + "] 在数据库中，不存在");
			}

		} catch (Exception e1) {
			e1.printStackTrace();
			j.setMessage(e1.getMessage());
	...
	public void dogenerateOne2Many(CodeParamEntity codeParamEntityIn, GenerateSubListEntity subTableListEntity, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			j.setMessage("成功生成增删改查->功能：" + codeParamEntityIn.getFtlDescription());
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void getCgformConfig(CgFormHeadEntity cgFormHead, GenerateEntity generateEntity) throws Exception {
		...
		for (CgFormFieldEntity field : cgFormHead.getColumns()) {
			...
			if (listJsCopy != null) {
				listJsCopy.setCgJsStr(listJsCopy.getCgJsStr().replace(fieldName, SbReadTable.formatField(fieldName)));
			}
			if (formJsCopy != null) {
				formJsCopy.setCgJsStr(formJsCopy.getCgJsStr().replace(fieldName, SbReadTable.formatField(fieldName)));
			}
		}
	...
	public ModelAndView goFileTree(HttpServletRequest request) {
		return new ModelAndView("sb/cgform/generate/fileTree");
	}
