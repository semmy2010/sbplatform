package com.sbplatform.web.cgform.controller.cgformftl;
...
import com.sbplatform.core.util.LogUtil;
...
import com.sbplatform.web.cgform.common.OfficeHtmlUtil;
...
public class CgformFtlController extends BaseController {
	...
	public ModelAndView ckeditor(HttpServletRequest request, String id) {
		...
		return new ModelAndView("sb/cgform/cgformftl/ckeditor");
	}
	...
	public ModelAndView cgformFtl(HttpServletRequest request) {
		...
		return new ModelAndView("sb/cgform/cgformftl/cgformFtlList");
	}
	...
	public ModelAndView addorupdate(CgformFtlEntity cgformFtl,
		...
		return new ModelAndView("sb/cgform/cgformftl/cgformFtl");
	}
	...
	public AjaxJson saveWordFiles(HttpServletRequest request,
		...
		if (StringUtil.isNotEmpty(message))
			j.setMessage("Word转Ftl失败," + message);
		else
			j.setMessage("Word转Ftl成功");
	...
	public ModelAndView cgformFtl2(HttpServletRequest request) {
		String formid = request.getParameter("formid");
		request.setAttribute("formid", formid);
		return new ModelAndView("sb/cgform/cgformftl/cgformFtlList2");
	}
	...
	public ModelAndView addorupdate2(CgformFtlEntity cgformFtl,
		...
		sb.append(".hide();}}}},callback:function(data){var win = frameElement.api.opener;if(data.status==0");
		sb.append("==0){frameElement.api.close();win.tip(data.message);}else{if(data.responseText==''||");
		sb.append("data.responseText==undefined)$(\"#formobj\").html(data.message);else $(\"#formobj\")");
		...
		req.setAttribute("cgformFtlPage", cgformFtl);
		return new ModelAndView("sb/cgform/cgformftl/cgformFtlEditor");
	}
