package com.sbplatform.web.cgform.controller.config;
...
import com.sbplatform.web.cgform.service.impl.config.util.FieldNumComparator;
...
public class CgFormHeadController extends BaseController {
	...
	public ModelAndView cgFormHead(HttpServletRequest request) {
		return new ModelAndView("sb/cgform/config/cgFormHeadList");
	}

	@RequestMapping(params = "goCgFormSynChoice")
	public ModelAndView goCgFormSynChoice(HttpServletRequest request) {
		return new ModelAndView("sb/cgform/config/cgformSynChoice");
	}
	...
	public void datagrid(CgFormHeadEntity cgFormHead, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		// 查询条件组装器
		com.sbplatform.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, cgFormHead);
	...
	public AjaxJson save(CgFormHeadEntity cgFormHead, HttpServletRequest request) {
		 ...
		 * .getJformPkType())){
		 * if((cgFormHead.getJformPkType().equalsIgnoreCase(
		 * "NATIVE")||cgFormHead.getJformPkType().equalsIgnoreCase("SEQUENCE"))
		 ...
		 * com.sbplatform.core.common.exception
		 * .BusinessException("数据表已经创建,不能更换主键策略"); } } } }
		 */
		//step.2 判定表格是否存在
	...
	public ModelAndView addorupdate(CgFormHeadEntity cgFormHead, HttpServletRequest req) {
		...
		return new ModelAndView("sb/cgform/config/cgFormHead");
	}
	...
	public AjaxJson checkIsExit(String name, HttpServletRequest req) {
		AjaxJson j = new AjaxJson();
		j.setStatus(cgFormFieldService.judgeTableIsExit(name) ? Constant.CommonConstant.STATUS_SUCCESS : Constant.CommonConstant.STATUS_FAILURE);
		return j;
	}
	...
	public ModelAndView sqlPlugin(String id, HttpServletRequest request) {
		...
		return new ModelAndView("sb/cgform/config/cgFormSqlPlugin");
	}
	...
	public ModelAndView jsPlugin(String id, HttpServletRequest request) {
		CgFormHeadEntity bean = cgFormFieldService.getEntity(CgFormHeadEntity.class, id);
		request.setAttribute("bean", bean);
		return new ModelAndView("sb/cgform/config/cgFormJsPlugin");
	}
