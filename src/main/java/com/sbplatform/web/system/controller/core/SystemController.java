...
import com.sbplatform.web.system.manager.ClientSort;
...
import com.sbplatform.web.system.pojo.base.Version;
...
public class SystemController extends BaseController {
	...
	public AjaxJson delTypeGridTree(String id, HttpServletRequest request) {
		...
		if (id.startsWith("G")) {//分组
			...
			message = "数据字典类型: " + type.getName() + "被删除 成功";
	...
	public AjaxJson delType(Type type, HttpServletRequest request) {
		...
		message = "类型: " + type.getName() + "被删除 成功";
		systemService.delete(type);
		//刷新缓存
	...
	public ValidForm checkType(HttpServletRequest request) {
		...
		hql.append(" AND entity.code =  '").append(typeCode).append("'");
	...
	public AjaxJson saveType(Type type, HttpServletRequest request) {
		...
		if (StringUtil.isNotEmpty(type.getId())) {
			message = "类型: " + type.getName() + "被更新成功";
			userService.saveOrUpdate(type);
			systemService.addLog(message, Globals.Log_Type_UPDATE, Globals.Log_Leavel_INFO);
		} else {
			message = "类型: " + type.getName() + "被添加成功";
	...
	@ResponseBody
	public AjaxJson delDepart(Department depart, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
	...
	@ResponseBody
	public AjaxJson saveDepart(Department depart, HttpServletRequest request) {
		// 设置上级部门
	...
	public ModelAndView addorupdateDepart(Department depart, HttpServletRequest req) {
	...
	public List<ComboTree> setAuthority(Role role, HttpServletRequest request, ComboTree comboTree) {
		...
		if (comboTree.getId() != null) {
			cq.eq("parent.id", oConvertUtils.getInt(comboTree.getId(), 0));
		}
		if (comboTree.getId() == null) {
			cq.isNull("parent");
		}
	...
	public String updateAuthority(HttpServletRequest request) {
		...
		if (rolefunction != "") {
			...
			for (String s : roleFunctions) {
				...
				MenuFunction f = this.systemService.get(MenuFunction.class, Integer.valueOf(s));
	...
	public List<TreeGrid> setOperate(HttpServletRequest request, TreeGrid treegrid) {
		...
		CriteriaQuery cq = new CriteriaQuery(MenuFunction.class);
		if (treegrid.getId() != null) {
			cq.eq("parent.id", oConvertUtils.getInt(treegrid.getId(), 0));
		}
		if (treegrid.getId() == null) {
			cq.isNull("parent");
		}
	...
	public void savep(String roleId, String functionId, String ids) {
		String hql = "from RoleMenuFunction t where" + " t.role.id=" + roleId + " " + "and t.menuFunction.id=" + functionId;
		RoleMenuFunction rFunction = systemService.singleResult(hql);
		if (rFunction != null) {
	...
	public void clearp(String roleid) {
		String hql = "from RoleMenuFunction t where" + " t.role.id=" + roleid;
		List<RoleMenuFunction> rFunctions = systemService.findByQueryString(hql);
	...
	public void documentList(HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		cq.createAlias("type", "type");
		cq.eq("type.code", typeCode);
		cq.add();
		this.systemService.getDataGridReturn(cq, true);
		TagUtil.datagrid(response, dataGrid);
	}
	...
	public AjaxJson saveFiles(HttpServletRequest request, HttpServletResponse response, Document document) {
		...
		Type type = systemService.getType("files","附件", typeGroup);
		...
		attributes.put("name", document.getAttachmentName());
