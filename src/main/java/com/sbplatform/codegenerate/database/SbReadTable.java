package com.sbplatform.codegenerate.database;
...
import com.sbplatform.codegenerate.pojo.TableConvert;
import com.sbplatform.codegenerate.util.CodeResourceUtil;
import com.sbplatform.codegenerate.util.CodeStringUtils;

public class SbReadTable {
	private static final Log log = LogFactory.getLog(SbReadTable.class);
	...
	public static void main(String[] args) throws SQLException {
		try {
			List<Columnt> cls = new SbReadTable().readTableColumn("person");
			for (Columnt c : cls)
				System.out.println(c.getFieldName());
		} catch (Exception e) {
			e.printStackTrace();
		}
		System.out.println(ArrayUtils.toString(new SbReadTable().readAllTableNames()));
	}
	...
	public List<String> readAllTableNames() throws SQLException {
		...
		try {
			...
			if (CodeResourceUtil.DATABASE_TYPE.equals("mysql")) {
				this.sql = MessageFormat.format("select distinct table_name from information_schema.columns where table_schema = {0}",
	...
	public List readTableColumn(String tableName) throws Exception {
		...
		try {
			...
			if (n > 0) {
				...
				if (CodeResourceUtil.SB_GENERATE_UI_FILTER_FIELDS != null)
					ui_filter_fields = CodeResourceUtil.SB_GENERATE_UI_FILTER_FIELDS.toLowerCase().split(",");
				if (!CodeResourceUtil.SB_GENERATE_TABLE_ID.equals(columnt.getFieldName()) && !CodeStringUtils.isIn(columnt.getFieldDbName().toLowerCase(), ui_filter_fields))
				...
				while (rs.previous()) {
					...
					if (!CodeResourceUtil.SB_GENERATE_TABLE_ID.equals(po.getFieldName()) && !CodeStringUtils.isIn(po.getFieldDbName().toLowerCase(), ui_filter_fields)) {
	...
	public boolean checkTableExist(String tableName) {
		try {
			System.out.println("��ݿ���: " + CodeResourceUtil.DIVER_NAME);
			Class.forName(CodeResourceUtil.DIVER_NAME);
			this.conn = DriverManager.getConnection(CodeResourceUtil.URL, CodeResourceUtil.USERNAME, CodeResourceUtil.PASSWORD);
			this.stmt = this.conn.createStatement(1005, 1007);

			if (CodeResourceUtil.DATABASE_TYPE.equals("mysql")) {
				this.sql = "select column_name,data_type,column_comment,0,0 from information_schema.columns where table_name = '" + tableName.toUpperCase() + "'" + " and table_schema = '"
