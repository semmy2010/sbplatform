				...
				if ($('#demotree').tree('isLeaf', node.target)) {
					$.ajax({
						...
						success:function(html){
							$("#demopanle").html(html+'<script type="text/javascript">$(function(){$("#demopanle").Validform({tiptype:4,btnSubmit:"#btn_sub",btnReset:"#btn_reset",ajaxPost:true,callback:function(data){var win = frameElement.api.opener;if(data.status==0){frameElement.api.close();win.tip(data.message);}else{'+
									'if(data.responseText==""||data.responseText==undefined)$("#demopanle").html(data.message);'+
