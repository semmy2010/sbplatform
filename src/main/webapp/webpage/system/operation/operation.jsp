...
<t:formvalid formid="formobj" layout="div" dialog="true" action="functionController.do?saveop">
	...
	<div class="form"><label class="Validform_label"> 图标名称: </label> <select name="icon.id">
		<c:forEach items="${iconlist}" var="icon">
			<option value="${icon.id}" <c:if test="${icon.id==function.icon.id }">selected="selected"</c:if>>${icon.iconName}</option>
		</c:forEach>
	</select></div>
	<input name="menuFunction.id" value="${functionId}" type="hidden">
