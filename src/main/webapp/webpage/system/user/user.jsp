...
<t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="userController.do?saveUser">
	...
	<table cellpadding="0" cellspacing="1" class="formtable">
     ...
     ${user.userName }
				...
				<input id="userName" class="inputxt" name="userName" validType="s_t_user,user_name,id" value="${user.userName }" datatype="s2-10" />
		...
		<tr>
			...
			<td class="value"><select id="department.id" name="department.id" datatype="*">
				<c:forEach items="${departList}" var="depart">
					<option value="${depart.id }" <c:if test="${depart.id==user.department.id}">selected="selected"</c:if>>${depart.name}</option>
				</c:forEach>
...
<script type="text/javascript">
	$(function(){
		...
		setInterval("console.log($('body').height())",2000);
		setInterval("console.log($(document).height())",2000);
	});
</script>
</body>