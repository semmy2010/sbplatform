...
public class ${entityName}ServiceImpl extends CommonServiceImpl implements ${entityName}ServiceI {
	...
	public void addMain(${entityName}Entity ${entityName?uncap_first},
			...
			for(${sub.entityName}Entity ${sub.entityName?uncap_first}:${sub.entityName?uncap_first}List){
				...
				//外键设置
				<#if key?lower_case?index_of("${sb_table_id}")!=-1>
				${sub.entityName?uncap_first}.set${subFieldMeta1[key]?cap_first}(${entityName?uncap_first}.get${sb_table_id?cap_first}());
				<#else>
	...
	public void updateMain(${entityName}Entity ${entityName?uncap_first},
	    ...
	    <#list subTab as sub>
		    <#list sub.foreignKeys as key>
		    <#if key?lower_case?index_of("${sb_table_id}")!=-1>
		Object ${sb_table_id}${sub_index} = ${entityName?uncap_first}.get${sb_table_id?cap_first}();
		    <#else>
	    ...
	    List<${sub.entityName}Entity> ${sub.entityName?uncap_first}OldList = this.findHql(hql${sub_index},<#list sub.foreignKeys as key><#if key?lower_case?index_of("${sb_table_id}")!=-1>${sb_table_id}${sub_index}<#else>${key?uncap_first}${sub_index}</#if><#if key_has_next>,</#if></#list>);
